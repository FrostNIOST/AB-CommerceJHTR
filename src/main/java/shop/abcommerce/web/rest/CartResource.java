...
/**
 * REST controller for managing {@link shop.abcommerce.domain.Cart}.
 */
@RestController
...
public class CartResource {
    ...
    public ResponseEntity<CartDTO> createCart(@Valid @RequestBody CartDTO cartDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/carts/" + cartDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, cartDTO.getId()))
            .body(cartDTO);
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<CartDTO> getCart(@PathVariable("id") String id) {
