...
/**
 * REST controller for managing {@link shop.abcommerce.domain.CartItem}.
 */
@RestController
...
public class CartItemResource {
    ...
    public ResponseEntity<CartItemDTO> createCartItem(@Valid @RequestBody CartItemDTO cartItemDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/cart-items/" + cartItemDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, cartItemDTO.getId()))
            .body(cartItemDTO);
    }
    ...
    public ResponseEntity<CartItemDTO> updateCartItem(
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, cartItemDTO.getId()))
            .body(cartItemDTO);
    }
    ...
    public ResponseEntity<CartItemDTO> partialUpdateCartItem(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, cartItemDTO.getId())
        );
    }
    ...
    public ResponseEntity<List<CartItemDTO>> getAllCartItems(
        ...
        Page<CartItemDTO> page;
        if (eagerload) {
            page = cartItemService.findAllWithEagerRelationships(pageable);
        } else {
            page = cartItemService.findAll(pageable);
        }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<CartItemDTO> getCartItem(@PathVariable("id") String id) {
