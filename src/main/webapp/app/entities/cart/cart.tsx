      ...
      <div className="table-responsive">
        {cartList?.length > 0 ? (
          <Table responsive>
            ...
            <tbody>
              {cartList.map(cart => (
                <tr key={`entity-${cart.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/cart/${cart.id}`} variant="link" size="sm">
                      {cart.id}
                    </Button>
                  </td>
                  <td>{cart.state}</td>
                  <td>{cart.account ? <Link to={`/account-u/${cart.account.id}`}>{cart.account.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/cart/${cart.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/cart/${cart.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Ningún Carts encontrado</div>
        )}
      </div>
      {totalItems ? (
