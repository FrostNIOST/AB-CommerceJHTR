    ...
    <div>
      ...
      <div className="table-responsive">
        {orderList?.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('orderNumber')}>
                  Order Number <FontAwesomeIcon icon={getSortIconByFieldName('orderNumber')} />
                </th>
                <th className="hand" onClick={sort('subtotal')}>
                  Subtotal <FontAwesomeIcon icon={getSortIconByFieldName('subtotal')} />
                </th>
                <th className="hand" onClick={sort('subtotalIva')}>
                  Subtotal Iva <FontAwesomeIcon icon={getSortIconByFieldName('subtotalIva')} />
                </th>
                <th className="hand" onClick={sort('shippingCost')}>
                  Shipping Cost <FontAwesomeIcon icon={getSortIconByFieldName('shippingCost')} />
                </th>
            ...
            <tbody>
              {orderList.map(order => (
                <tr key={`entity-${order.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/order/${order.id}`} variant="link" size="sm">
                      {order.id}
                    </Button>
                  </td>
                  <td>{order.orderNumber}</td>
                  <td>{order.subtotal}</td>
                  <td>{order.subtotalIva}</td>
                  ...
                  <td>
                    {order.paymentMethod ? (
                      <Link to={`/payment-method/${order.paymentMethod.id}`}>{order.paymentMethod.namePaymentMethod}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {order.shippingAddress ? <Link to={`/address/${order.shippingAddress.id}`}>{order.shippingAddress.adress}</Link> : ''}
                  </td>
                  <td>{order.account ? <Link to={`/account-u/${order.account.id}`}>{order.account.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/order/${order.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/order/${order.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Ningún Orders encontrado</div>
        )}
      </div>
      {totalItems ? (
