    ...
    <div>
      ...
      <div className="table-responsive">
        {paymentMethodList?.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('namePaymentMethod')}>
                  Name Payment Method <FontAwesomeIcon icon={getSortIconByFieldName('namePaymentMethod')} />
                </th>
                <th className="hand" onClick={sort('cod')}>
                  Cod <FontAwesomeIcon icon={getSortIconByFieldName('cod')} />
                </th>
                ...
                <th className="hand" onClick={sort('apiKey')}>
                  Api Key <FontAwesomeIcon icon={getSortIconByFieldName('apiKey')} />
                </th>
                <th className="hand" onClick={sort('endpoint')}>
                  Endpoint <FontAwesomeIcon icon={getSortIconByFieldName('endpoint')} />
                </th>
            ...
            <tbody>
              {paymentMethodList.map(paymentMethod => (
                <tr key={`entity-${paymentMethod.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/payment-method/${paymentMethod.id}`} variant="link" size="sm">
                      {paymentMethod.id}
                    </Button>
                  </td>
                  <td>{paymentMethod.namePaymentMethod}</td>
                  <td>{paymentMethod.cod}</td>
                  <td>{paymentMethod.description}</td>
                  <td>{paymentMethod.type}</td>
                  <td>{paymentMethod.apiKey}</td>
                  <td>{paymentMethod.endpoint}</td>
                  <td>{paymentMethod.state}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button
                        as={Link as any}
                        to={`/payment-method/${paymentMethod.id}`}
                        variant="info"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/payment-method/${paymentMethod.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Ningún Payment Methods encontrado</div>
        )}
      </div>
      {totalItems ? (
