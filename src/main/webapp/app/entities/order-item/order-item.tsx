    ...
    <div>
      ...
      <div className="table-responsive">
        {orderItemList?.length > 0 ? (
          <Table responsive>
            ...
            <tbody>
              {orderItemList.map(orderItem => (
                <tr key={`entity-${orderItem.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/order-item/${orderItem.id}`} variant="link" size="sm">
                      {orderItem.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/order-item/${orderItem.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/order-item/${orderItem.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Ningún Order Items encontrado</div>
        )}
      </div>
      {totalItems ? (
