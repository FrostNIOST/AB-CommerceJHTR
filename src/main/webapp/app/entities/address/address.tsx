    ...
    <div>
      ...
      <div className="table-responsive">
        {addressList?.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('adress')}>
                  Adress <FontAwesomeIcon icon={getSortIconByFieldName('adress')} />
                </th>
            ...
            <tbody>
              {addressList.map(address => (
                <tr key={`entity-${address.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/address/${address.id}`} variant="link" size="sm">
                      {address.id}
                    </Button>
                  </td>
                  ...
                  <td>{address.state}</td>
                  <td>{address.account ? <Link to={`/account-u/${address.account.id}`}>{address.account.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/address/${address.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/address/${address.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Ningún Addresses encontrado</div>
        )}
      </div>
      {totalItems ? (
