    ...
    <div>
      ...
      <div className="table-responsive">
        {cartItemList?.length > 0 ? (
          <Table responsive>
            ...
            <tbody>
              {cartItemList.map(cartItem => (
                <tr key={`entity-${cartItem.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/cart-item/${cartItem.id}`} variant="link" size="sm">
                      {cartItem.id}
                    </Button>
                  </td>
                  <td>{cartItem.quantity}</td>
                  <td>{cartItem.product ? <Link to={`/product/${cartItem.product.id}`}>{cartItem.product.productName}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/cart-item/${cartItem.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/cart-item/${cartItem.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Ningún Cart Items encontrado</div>
        )}
      </div>
      {totalItems ? (
